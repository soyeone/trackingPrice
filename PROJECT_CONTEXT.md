...
* **카테고리 자동 분류 & 보강 (Auto-Categorization)**:
  - 올리브영 모바일 상세 페이지의 Breadcrumb 및 메타데이터를 파싱하여 12개 정규 카테고리(스킨케어, 마스크팩, 클렌징, 선케어 등)를 **자동 식별**합니다.
  - 사용자가 단순 공유 링크(카테고리 정보가 없는 URL)로 등록하더라도 등록 폼에서 카테고리가 자동 선택되며, DB 저장 시 URL에 `&catName=...&dispCatNo=...`가 자동으로 결합됩니다.
  - 등록 시 카테고리가 누락된 기존 상품이 있더라도, **다음 일일 트래킹(`daily_crawler.js`)이 실행될 때 모바일 상세 페이지에서 카테고리를 자동 추출하여 DB의 URL을 올바른 카테고리로 자동 갱신/조정**합니다.
* `server.js` 실행 환경에서는 `/api/scrape?goodsNo=...`를 통해 제품명, 카테고리, 현재 가격, 단위가격을 자동으로 채워 넣고 원클릭 등록합니다.
