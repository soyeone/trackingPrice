...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$dir = '%TARGET_DIR%'.TrimEnd('\'); (Get-Content 'scraper\autostart_template.vbs' -Raw).Replace('{DIR}', $dir) | Set-Content -Path ($env:APPDATA + '\Microsoft\Windows\Start Menu\Programs\Startup\OliveYoungPriceTracker.vbs') -Encoding Ascii"

if not exist "%VBS_LINK%" goto :ERROR
...
echo  - 등록 위치: %VBS_LINK%
echo  - 동작 방식:
echo     [1] PC가 켜지거나 로그인할 때 수집 콘솔 창이 열려 실시간 진행상황(진행률/예상시간)을 표시합니다.
